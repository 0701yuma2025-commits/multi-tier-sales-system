...
class SupabaseDatabase {
    constructor() {
        ...
        if (!this.supabaseUrl || !this.supabaseAnonKey) {
            console.warn('Supabase環境変数が設定されていません。LocalStorageモードで動作します。');
    ...
    async request(endpoint, options = {}) {
        if (this.useLocalStorage) {
            throw new Error('LocalStorageモードです。Supabase環境変数を設定してください。');
        }
    ...
    async getSales(filters = {}) {
        try {
            ...
            // LocalStorageにフォールバック
            if (window.db) {
                return window.db.getSales(filters);
            }
            return [];
        }
    }
    ...
    async addSale(saleData) {
        try {
            ...
            // LocalStorageにフォールバック
            if (window.db) {
                return window.db.addSale(saleData);
            }
            throw error;
        }
    }
    ...
    async deleteSale(saleId) {
        try {
            ...
            // LocalStorageにフォールバック
            if (window.db) {
                return window.db.deleteSale(saleId);
            }
            throw error;
        }
    }
    ...
    async getSalesStats() {
        try {
            ...
            // LocalStorageにフォールバック
            if (window.db) {
                return window.db.getSalesStats();
            }
    ...
    async login(email, password) {
        if (this.useLocalStorage) {
            // LocalStorageモード：簡易認証
            if ((email === 'admin@example.com' && password === 'admin123') ||
                (email === 'agency@example.com' && password === 'agency123')) {
                return { success: true, role: email.includes('admin') ? 'admin' : 'agency' };
            }
